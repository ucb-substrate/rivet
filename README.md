...
- Hierarchical:
  - Parametrize reference flow with tree of hierarchical modules and per flow configuration
  - Per flow configuration is a struct like this:
    ```rust
    struct FlowConfig {
        // Maybe change to enum if it doesn't make sense to have both a syn and par checkpoint.
        syn: Option<Checkpoint>,
        par: Option<Checkpoint>,
        linear_flow_params: LinearFlowParams,
    }
    ```
  - Parameterize linear flow
    - ability to place macros by passing arguments to the `reference_flow` function, rather than fiddling with the output `Flow` e.g. sram placement constraints
